...
/** `active` = the tool in use (persistent selection). `open` = a panel this
 *  button revealed (disclosure), which gets a plainer treatment so the toolbar
 *  never shows two things that look equally selected. */
function LockInWorkspaceIconButton({ label, active = false, open = false, children, className = "", ...props }) {
  return <button
    className={`workspace-v2-icon-button${active ? " is-active" : ""}${open ? " is-open" : ""}${className ? ` ${className}` : ""}`}
    type="button"
    aria-label={label}
    title={label}
    {...props}
  ...
  return (
    <section className={`lockin-reference-viewer workspace-v2-reader${isFullscreen ? " is-document-fullscreen" : ""}`} ref={viewerRef} aria-label="Study document">
      ...
      <nav className="workspace-v2-toolbar lockin-workspace-tools" aria-label="Document tools">
        <div className="workspace-v2-tool-list">{LOCK_IN_VIEWER_TOOLS.map(([value, label, ToolIcon]) => <LockInWorkspaceIconButton key={value} label={label} active={activeTool === value} aria-pressed={activeTool === value} onClick={() => selectTool(value)}><ToolIcon size={19} /></LockInWorkspaceIconButton>)}</div>
