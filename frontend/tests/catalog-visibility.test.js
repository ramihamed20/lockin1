...
test("every catalogue screen separates 'not loaded yet' from 'nothing here'", async () => {
  ...
  // Materials, CatalogMaterialSheets and CatalogSheetStudy live here. The
  // Sheet Summary is now the study reader in summary mode, so it gates in the
  // workspace with every other reader screen.
  const gates = materials.match(/if \(loading\) return/g) || [];
  assert.equal(gates.length, 3, "each catalogue screen gates on loading");
  const errorGates = materials.match(/if \(error\) return/g) || [];
  assert.equal(errorGates.length, 3, "each screen offers a retry instead of 404");
