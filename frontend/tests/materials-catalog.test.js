...
test("Catalog Focus Workspace uses a compact contextual toolbar and persistent catalog bookmarks", async () => {
  ...
  assert.match(workspace, /<ActiveStudyPlayer sheetId=\{sheet\.learningObjectId\}/);
  assert.doesNotMatch(workspace, /focusApi\.startActiveStudy/);
  const activeStudyPlayer = await readFile(new URL("../src/components/learning/ActiveStudyPlayer.jsx", import.meta.url), "utf8");
  assert.match(activeStudyPlayer, /focusApi\.startManagedActiveStudy/);
  assert.match(activeStudyPlayer, /getManagedActiveStudyQuestions/);
  assert.match(activeStudyPlayer, /submitManagedActiveStudy/);
  assert.match(activeStudyPlayer, /"abandon"/);
  assert.match(activeStudyPlayer, /\? 50 : 15/);
