...
def seed_libyan_tree(apps, schema_editor):  # type: ignore[no-untyped-def]
    ...
    for college_key, (code, name_en, name_ar, position) in dentistry_programs.items():
        ...
        for year_code, year_title, year_name_ar, subjects, year_position in (
            ("year-1", "First Year", "سنة أولى", first_year_subjects, 1),
            ("year-2", "Second Year", "سنة ثانية", second_year_subjects, 2),
        ):
            ...
            cohort, _ = StudentCohort.objects.update_or_create(
                ...
                defaults={
                    "program": program,
                    "code": year_code,
                    "name_en": f"{name_en} — {year_title}",
                    "name_ar": f"{name_ar} {year_name_ar}",
                    "is_active": True,
    ...
    medicine_department = node(
        colleges["tripoli"], "department", "human-medicine", "Human Medicine", 2
    )
