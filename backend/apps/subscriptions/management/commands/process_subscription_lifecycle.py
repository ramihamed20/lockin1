...
class Command(BaseCommand):
    ...
    def handle(self, *args: object, **options: object) -> None:
        ...
        for subscription in queryset.filter(
            ...
            except Exception as error:  # noqa: BLE001 - one account must not stop the run
                # This job is what expires trials and warns readers before their
                # access ends. One unadvanceable subscription used to abort the
                # pass, so every account behind it silently kept access it had
                # stopped paying for, and got no warning either.
                failures += 1
                logger.exception(
                    "Subscription lifecycle skipped a subscription",
                    extra={"subscription_id": str(subscription.id)},
                )
                providers.metric_sink.increment("subscriptions.lifecycle.record_failed")
                self.stderr.write(f"subscription {subscription.id} could not advance: {error}")
                continue
        ...
        if failures:
            raise CommandError(
                f"{failures} subscription(s) could not advance; see lockin.jobs logs."
            )

    def _advance(self, *, subscription: Subscription, now: datetime) -> tuple[int, int]:
        transitioned = 0
        reminders = 0
