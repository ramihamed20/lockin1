...
def learning_dashboard(*, user: User) -> dict[str, object]:
    ...
    recent_bookmark = bookmarks_for_user(user=user).filter(learning_object__isnull=False).first()
    ...
    if next_progress is not None:
        ...
        if version is not None:
            next_item = {
                ...
                "focus_document_version_id": version.id if version.content_type == "pdf" else None,
                "title": version.title,
                "content_type": version.content_type,
                "reason": "resume",
                "completion_percent": next_progress.completion_percent,
            }
    elif recent_bookmark is not None and recent_bookmark.learning_object is not None:
